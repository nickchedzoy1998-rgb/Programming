...
def most_goals(ball_players: list) -> str:
    goals_sorted = sorted(ball_players, key=lambda x: x.goals, reverse=True)

    return goals_sorted[0].name

def most_points(ball_players: list) -> tuple:
    player_points = sorted(ball_players, key=lambda x: x.goals + x.passes, reverse=True)

    return (player_points[0].name, player_points[0].number)

def least_minutes(ball_players: list) -> object:
    least_mins_sorted = sorted(ball_players, key=lambda x: x.minutes)

    return least_mins_sorted[0]
